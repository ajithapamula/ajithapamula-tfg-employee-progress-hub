...
CREATE POLICY "Insert own report" ON public.daily_reports FOR INSERT TO authenticated WITH CHECK (auth.uid() = user_id);
CREATE POLICY "Update own report" ON public.daily_reports FOR UPDATE TO authenticated USING (auth.uid() = user_id);
CREATE POLICY "Delete own report" ON public.daily_reports FOR DELETE TO authenticated USING (auth.uid() = user_id);
